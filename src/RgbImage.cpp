...
// Allow the use of deprecated fopen() instead of fopen_s()
...
bool RgbImage::LoadFromOpenglBuffer()					// Load the bitmap from the current OpenGL buffer
{
    ...
    int oldGlUnpackAlign;
    glGetIntegerv(GL_UNPACK_ALIGNMENT, &oldGlUnpackAlign);
    if (oldGlUnpackAlign != 4) {
	...
	// Restore the row length and unpack alignment in glPixelStorei as needed.
	if ( vWidth>=NumCols ) {
		glPixelStorei( GL_UNPACK_ROW_LENGTH, oldGlRowLen );
	}
    if (oldGlUnpackAlign != 4) {
        glPixelStorei(GL_UNPACK_ALIGNMENT, oldGlUnpackAlign);
    }
	return true;
}
